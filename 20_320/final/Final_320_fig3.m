...
for i = 2:length(interp_all_data)
    ...
    % to_be_fit = [Alpha, Beta, k_o, k, Io/Xo, p_c] these are the coefficients that I want to fit
    % with lsqcurvefit
    ...
    %and I(t)
    to_be_fit = [Alpha, Beta, 0.01,0.01,2,0.01];
...
%     options2.OptimalityTolerance = 1e-8;
  ...
  %  options2.FunctionTolerance = 1e-9;
    ...
    ub = [0.849,0.3922 inf inf inf inf];
...
%parameters as inputs. But both are passed into y0 for this function.
    ...
    %y0 = [Alpha,Beta,k_o,k,Io/Xo,X_t_o,N,p_c,time]
    N = y0(7);
    %y = [S,I,R,X]
    y = [1-y0(6)-y0(5).*y0(6), y0(5).*y0(6), 0, y0(6)];
    %k = [Alpha, Beta, k_o, k,p_c]
    ...
    dydt(1,:) = -((Alpha_.*S.*I)) - (1/(p_c.*(S+1)^2)).*k_o.*S;
    dydt(2,:) = (Alpha_.*S.*I) - Beta_.*I - I.*(k_o+k_).*(1/(p_c.*((S+1)^2)));
...
function [plotOut,X_t_Data_at_feb_12] = Plot_infection(y0,~)
    %y0 = [Alpha,Beta,k_o,k,Io/Xo,X_t_o,p_c]
