...
X_fitted_2A = lsqcurvefit(infection_fit,to_be_fit,hubei_data(2,:), hubei_data(1,:),[0.55,0.0490,0,0,10^-3],[0.849,0.3922,0.2,0.2,100],options2);
...
all_r_t{i} = {zeros(1:length(interp_all_data))};
...
for i = 2:length(interp_all_data)
    ...
    %and I(t)
    to_be_fit = [Alpha, Beta, 0.01,0.01,2];

    %infection_fit is a dummy function that is used to initialize all the
    %initial conditions I don't want to fit without passing them through
    %lsqcurvefit
  
    infection_fit = @(p,t) infection([p(1:5), X_t_o, N(i), points{i}],t);
...
%     options2.MaxIterations = 1e6;
    ...
    %output equations
    plot_infection_fit = @(p,t) Plot_infection([p(1:5), X_t_o, points{i}]);
    ...
    all_r_t{i} = Plotting_data.y(3,:).*N(i);
...
for i = 2:length(all_X_t)
    ...
    hold on
    plot(all_time{i}, all_r_t{i},'-r', "LineWidth", 1.5)
...
%parameters as inputs. But both are passed into y0 for this function.
    ...
    %y0 = [Alpha,Beta,k_o,k,Io/Xo,X_t_o,N,time]
    N = y0(7);
    %y = [S,I,R,X]
    ...
    function dydt = infectionODEs(~,y,k)
        ...
        dydt(2,:) = Alpha_.*S.*I - Beta_.*I - k_o.*I - k_.*I;
