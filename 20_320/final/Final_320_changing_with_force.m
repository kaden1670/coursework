%20.320 Final Project Part 2
%Kaden DiMarco

%implementation: Changing rates! In project 2, I implemented changing zombie
%destruction rates as people got better at removing zombies. The same concept can be applied here. As an epidemic
%progresses, containment policies continue to develop to better hinder the
%spread of infection. Here I will implement changing alpha, k, and k0
%values over time.
...
% to_be_fit = [Alpha,Beta,k_o, k, Io/Xo, Alpha_mod, k_o_mod, k_mod] these are the coefficients that I want to fit
% with lsqcurvefit
to_be_fit = [Alpha,Beta,0.1,0.1,1,0.01,0.01,0.01];
...
infection_fit = @(p,t) infection([p(1:5), X_t_o, N, p(6:8), hubei_data(2,:)],t);


options2 = optimoptions(@lsqcurvefit);
options2.MaxFunctionEvaluations = 1e4;

lb = [0.55,0.0490,0,0,10^-3,0,0,0];
ub = [0.849,0.3922,1,1,100,0.2,0.2,0.2];

% this returns the fitted parameters from to_be_fit
X_fitted_2A = lsqcurvefit(infection_fit,to_be_fit,hubei_data(2,:), hubei_data(1,:),lb,ub,options2);
...
plot_infection_fit = @(p,t) Plot_infection([p(1:5), X_t_o,N, p(6:8),hubei_data(2,:)]);
...
for i = 2:length(interp_all_data)
    ...
    %and I(t)
    % to_be_fit = [Alpha,Beta,k_o, k, Io/Xo, k_o_mod, k_mod, p] these are the coefficients that I want to fit
    % with lsqcurvefit
    %code runs faster if guess is the fitted values from part 1
    if i == 2
        to_be_fit = [Alpha,Beta, 0.094, 0.009, 1.39,0,0,0,];
    elseif i == 3
        to_be_fit = [Alpha,Beta, 0.088, 4.01e-7, 5.89,0,0,0];
    elseif i == 4
        to_be_fit = [Alpha,Beta,0.103, 0.034, 3.56,0,0,0];
    elseif i == 5
        to_be_fit = [Alpha,Beta,0.101, 0.036, 4.97,0,0,0];
    elseif i == 6
        to_be_fit = [Alpha,Beta,0.075,0.085,11.63,0,0,0];
    
    elseif i == 7
        to_be_fit = [Alpha,Beta,0.086, 0.052, 5.95,0,0,0];
    elseif i == 8
        to_be_fit = [Alpha,Beta,0.107, 0.004, 9.87,0,0,0];
    elseif i == 9
        to_be_fit = [Alpha,Beta,0.122, 0.002, 3.09,0,0,0];
    ...
    infection_fit = @(p,t) infection([p(1:5), X_t_o, N(i),p(6:8),points{i}],t);
...
%     options2.OptimalityTolerance = 1e-8;
    %options2.StepTolerance = 1e-8;
    %options2.FunctionTolerance = 1e-8;
    options2.MaxFunctionEvaluations = 1e4;
%     options2.MaxIterations = 1e6;
    ...
    %output equations
    plot_infection_fit = @(p,t) Plot_infection([p(1:5), X_t_o,N(i),p(6:8), points{i}]);
...
title(han,'Figure 2C Reproduction','FontSize',24);
...
%     options2.FunctionTolerance = 1e-10;
    ...
    options2.UseParallel = true; %can use if code slow
    %options2.FiniteDifferenceStepSize = 5e-6;
    lb = [0.55 0.0490 0 0 10^-3,0,0,0,0];
    
    %ub just helps code run faster/ a bit more accurate
    ub = [0.849,0.3922 1 1 100,0.5,0.5,0.5,1];
    ...
    %plot infection is the same function as infection but it returns all
    %output equations
    plot_infection_fit = @(p,t) Plot_infection([p(1:5), X_t_o,N(i),p(6:9), points{i}]);
...
%parameters as inputs. But both are passed into y0 for this function.
    ...
    %y0 = [Alpha,Beta,k_o,k,Io/Xo,X_t_o,N,k_o_mod,k_mod,p,time]
    N = y0(7);
    %y = [S,I,R,X,k_o,k]
    y = [1-y0(6)-y0(5).*y0(6), y0(5).*y0(6),0,y0(6),y0(3),y0(4)];
    %k = [Alpha, Beta, k_o_mod, k_mod, p]
    k = [y0(1),y0(2),y0(8),y0(9),y0(10)];
    city_data = y0(11:length(y0));
    ...
    % Inputs:
    %
    % y: a 1x7 [S, I, R, X, Alpha, k, k_o]
    %   S - Susceptible population
    %   I - Infected population
    %   R - Removed population
    %   X - Symptomatic, quarantined population
    %   Alpha - modified alpha value
    %   k - modified k value
    %   k0 - modified k0 value
    %
    % k: a 1x5 [Alpha_mod, Beta, k_o_mod, k_mod, p] 
    %                   Alpha_mod  - Rate alpha changes, or rate
    %                                people reduce contacts
    %                   Beta       - Recovery rate
    %                   k_mod      - Rate that Removal rate (I -> X) increases (from
    %                                quarantine measures that only affect
    %                                susceptible individuals)    %  
    %                   k_o_mod    - Rate that Containment rate changes (influenced by social
    %                                distancing emphasis increasing over time)
                          
    % Output (output is dydt):
    % A 1X7  [dS/dt, dI/dt, dR/dt, dX/dt, dk_o/dt, dk/dt]
    ...
    dydt(1,:) = -((Alpha.*S.*I)./(1+p.*I^2)) - k_o.*S;
    dydt(2,:) = ((Alpha.*S.*I)./(1+p.*I^2)) - Beta_.*I - k_o.*I - k_.*I;
...
function [plotOut,X_t_Data_at_feb_12] = Plot_infection(y0,~)
    ...
    out = ode45(@Plot_infectionODEs, [0 40], y, options, k);
