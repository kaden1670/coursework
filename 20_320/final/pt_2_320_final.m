...
%     options2.FunctionTolerance = 1e-10;
    ...
    %ub just helps code run faster/ a bit more accurate
    ub = [0.849,0.3922 1 1 100,0.5,0.5,0.5];
...
%parameters as inputs. But both are passed into y0 for this function.
    ...
    %y0 = [Alpha,Beta,k_o,k,Io/Xo,X_t_o,N,Alpha_mod,k_o_mod,k_mod,time]
    N = y0(7);
    %y = [S,I,R,X,Alpha,k_o,k]
    y = [1-y0(6)-y0(5).*y0(6), y0(5).*y0(6), 0, y0(6), y0(1),y0(3),y0(4)];
    %k = [Alpha_mod, Beta, k_o_mod, k_mod]
    k = [y0(8),y0(2),y0(9),y0(10)];
    ...
    %
    % k: a 1x4 [Alpha_mod, Beta, k_o_mod, k_mod] 
    ...
    % A 1X7  [dS/dt, dI/dt, dR/dt, dX/dt, dAlpha/dt, dk_o/dt, dk/dt]
    ...
    dydt(1,:) = -Alpha.*S.*I - k_o.*S;
    dydt(2,:) = Alpha.*S.*I - Beta_.*I - k_o.*I - k_.*I;
    dydt(3,:) = Beta_.*I + k_o.*S;
    dydt(4,:) = (k_ + k_o).*I;
    dydt(5,:) = -Alpha.*Alpha_mod;
    dydt(6,:) = k_o.*k_o_mod;
    dydt(7,:) = k_.*k_mod;
